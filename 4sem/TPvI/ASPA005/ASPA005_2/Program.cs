...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        using (IRepository repository = Repository.Create("Celebrities"))
        {
            ...
            {
                // ¬ызываем метод обновлени€, который возвращает int? (id обновленной записи или null)
                int? updatedId = repository.updCelebrityById(id, celebrity);

                // ѕровер€ем, что обновление прошло успешно (updatedId не null)
                if (updatedId == null)
                    throw new UpdateByIdException($"UPDATE /Celebrities error, id = {id}");

                // —охран€ем изменени€ в репозитории
                if (repository.SaveChanges() <= 0)
                    throw new SaveException("/Celebrities error, SaveChanges() <= 0");

                // ¬озвращаем обновленный объект Celebrity с корректным Id (заглавна€ буква!)
                return new Celebrity(
                    (int)updatedId, // ѕриводим int? к int, так как проверка на null уже пройдена
                    celebrity.Firstname,
                    celebrity.Surname,
                    celebrity.PhotoPath
                );
            ...
            {
                //если параметр celebrity == null, то 500 с по€снением
                //≈сли Surname == null или его длина < 2, то 409 с по€снени€ми
            ...
            {
                //если параметр celebrity == null, то 500 с по€снением
                //если есть уже такой Surname, то 409 с по€снени€ми
                Celebrity? celebrity = context.Arguments.OfType<Celebrity>().FirstOrDefault();
            ...
            {
                //если параметр celebrity == null, то 500 с по€снением
                //если нет фото по BasePath + вывод названи€ файла NotFound
