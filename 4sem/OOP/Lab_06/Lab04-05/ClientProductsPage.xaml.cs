...
namespace Lab04_05
{
    public partial class ClientProductsPage : Page
    {
        ...
        private void OnThemeChanged(object sender, EventArgs e)
        {
            ...
            // Пересоздаем карточки
            CreateProductCards();
        }

        private void UpdateBackground()
        {
            var background = Settings.CurrentTheme == Settings.Themes.Dark
                ? (Brush)Application.Current.FindResource("MaterialDesignPaper")
                : Brushes.White;
        ...
        private void CreateProductCards()
        {
            ...
            var cardBackground = (Brush)Application.Current.FindResource("CardBackground");
            var cardTextBrush = (Brush)Application.Current.FindResource("CardTextBrush");

            foreach (var product in _products.Where(p => p.isActive == 1))
            {
                //products.Children.Add(CreateProductCard(product));
                products.Children.Add(CreateProductCard(product, cardBackground, cardTextBrush));
            }
        }

        //private UIElement CreateProductCard(Product product)
        private UIElement CreateProductCard(Product product, Brush cardBackground, Brush cardTextBrush)
        {
            var card = new Border
            {
            ...
            contentStack.Children.Add(new TextBlock
            {
                ...
                Foreground = cardTextBrush // Добавлено
            });
        ...
        {
            CreateProductCards(); // Пересоздаем карточки
        }
        //private void OnThemeChanged(object sender, EventArgs e)
        //{
        //    // Обновляем фон страницы
