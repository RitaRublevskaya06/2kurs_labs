...
namespace LA
{
	// Функция для поиска и анализа лексем в исходном тексте
	void FindLex(const In::IN& source, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		char* temp = new char[token_size] {};	// Создаем временный массив для хранения токенов
		int str_number = 1;
		int str_position = 1;
		In::IN in;
		...
		{
			// Проверка символов, которые не являются разделителями или операторами
			...
			{
				// Если накоплен токен, анализируем его
				if (j != 0)
				{
					i--;	// Сдвиг позиции назад, чтобы повторно обработать текущий символ
					temp[j] = '\0';
					if (Analyse(temp, str_number, lexTable, idTable))	// Если токен успешно проанализирован
					{
						temp[0] = '\0'; j = 0;
						continue;
					}
				...
				{
					// Обработка строковых литералов, заключенных в одинарные кавычки
					if (source.text[i] == '\'')
					{
						temp[j++] = source.text[i++];
			...
			// Если найден символ новой строки '|', обновляем номер строки
			if (source.text[i] == '|')
			{
				str_number++;
				str_position = 0;
	...
	// Функция для анализа и классификации токенов
	bool Analyse(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		static TypeOfVar flag_type_variable;	// Переменная для отслеживания типа переменной
		...
		// Обработка строкового литерала, заключенного в одинарные кавычки
		case '\'':
		{
		...
		// Обработка числового литерала и идентификаторов
		default:
		{
			...
			if (FST::execute(*number_literal))
			{
				...
				else
				{
					...
					idTable.table[idTable.size - 1].value.vint = atoi(token);	// Преобразуем строку токена в целое число и сохраняем его значение
					Add(lexTable, { LEX_LITERAL,str_number, idTable.size - 1 });
				}
			...
			else
			{
				...
				// Переходим к обработке идентификатора, если это не числовой литерал
				return AnalyseIdentificator(token, str_number, lexTable, idTable, flag_type_variable);
			}
		}
	...
	// Функция для анализа идентификаторов и проверки их области видимости
	bool AnalyseIdentificator(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable, TypeOfVar& flag_type_variable)
	{
		...
		if (FST::execute(*id))
		{
			bool Checked_id = false;	// Флаг, указывающий, был ли идентификатор проверен
					...
					flag_type_variable.LT_posititon == lexTable.size - 2))
			{
				if (strcmp(token, "main") == 0)
				{
					for (int iter = 0; iter < idTable.size; iter++)
					{
						...
						{
							throw ERROR_THROW_IN(129, str_number, -1);	// Ошибка при дублировании имени функции
						}
					}
				}
			...
			if (!Checked_id)
			{
				...
				for (int i = lexTable.size - 1; i >= 0; i--)
				{
						...
						idTable.GetEntry(lexTable.GetEntry(i).idxTI).idtype == IT::IDTYPE::F)
					{
						...
						{
							temp = idTable.IsId(token);		// Проверяем идентификатор без области видимости
