...
namespace IT
{
	...
	Entry::Entry()
	{
		...
		this->firstApi = 0;		//индекс первого появления
	...
	Entry::Entry(const char* parrent_function, const char* id, IDDATATYPE iddatatype, IDTYPE idtype, int first, int it)
	{
		...
		this->idtype = idtype;
		this->parmQuantity = 0;
		this->value.vint = it;	// Инициализирует целочисленное значение
	}
	Entry::Entry(const char* parrent_function, const char* id, IDDATATYPE iddatatype, IDTYPE idtype, int first, char* ch)
	{
		...
		this->idtype = idtype;
		this->parmQuantity = 0;
		strcpy_s(this->value.vstr.str, 255, ch);	// Копирует строковое значение в vstr
		this->value.vstr.len = strlen(ch);	// Устанавливает длину строки в vstr
	}
	...
	// Создает и инициализирует объект IdTable с заданным размером
	...
	// Добавляет новую запись в IdTable
	...
	// Проверяет, существует ли идентификатор в таблице, и возвращает его индекс, если он найден
	int IdTable::IsId(const char id[ID_MAXSIZE])
	{
		for (int iter = 0; iter < this->size; iter++)
		{
			if (strcmp(this->table[iter].id, id) == 0)	// Проверяет на совпадение
				return iter;
		}
		return TI_NULLIDX;
	}

	// Проверяет, существует ли идентификатор с определенной родительской функцией, и возвращает его индекс, если найден
	...
	// Генерирует уникальное имя для лексемы
	char* IdTable::GetLexemaName()
	{
		char buffer[3];		// Буфер для хранения числового значения
		char tempBuf[5];	// Временный буфер для итогового имени лексемы
		_itoa_s(this->noname_lexema_count, buffer, 10);		// Преобразует число в строку и сохраняет его в buffer
		tempBuf[0] = 'L';
		tempBuf[1] = '_';
		tempBuf[2] = buffer[0];
		tempBuf[3] = buffer[1];
	...
	// Метод для вывода таблицы идентификаторов на экран
	void IdTable::PrintIdTable(const wchar_t* in)
	{
		...
		char buffer[3];
		char tempBuf[5];
		int lexema_count = 0;	// Счетчик уникальных лексем
		bool exist = true;		// Флаг для проверки существования лексемы
		tempBuf[0] = 'L';
		tempBuf[1] = '_';
		...
		for (int i = 0; i < this->size; i++)
		{
			...
			case IT::IDTYPE::P:
				...
				//литерал
			case IT::IDTYPE::L:
				_itoa_s(lexema_count, buffer, 10);	// Преобразует счетчик лексем в строку и сохраняет в buffer
				...
				case 1:
					for (int k = 0; k < i; k++) {
						if (this->table[k].idtype == IT::IDTYPE::L)		// Проверяет наличие литерала среди уже добавленных
							if (this->table[k].value.vint == this->table[i].value.vint) {
								exist = true;	 // Устанавливает флаг, если найден дубликат
								break;
							}
					}
